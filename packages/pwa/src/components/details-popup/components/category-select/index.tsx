  ...
  return (
    <div className="grid grid-cols-5 gap-1">
      {options.map((item) => (
        ...
        >
          <div
            className={`w-11 h-11 rounded-full bg-${item.id === categoryId ? color : 'gray'}-100 flex items-center justify-center mb-1`}
          >
            <i
              className={clxs(
                `${item.icon} text-${item.id === categoryId ? color : 'gray'}-500 ri-lg`
              )}
            ></i>
          </div>
          <div
            className={clxs(
              `text-xs text-${item.id === categoryId ? color : 'gray'}-500 whitespace-nowrap overflow-hidden text-overflow-ellipsis`
            )}
          >
            {item.name}
          </div>
